...
> 面相对象(OO) => 功能模块[设计模式+算法(数据结构)] => 框架[使用多种设计模式] => 架构[服务器集群]
...
1. 单一职责
   ...
   + 提高类的可读性,可维护性
   + 降低变更引起的风险
   + 只有逻辑足够简单是才可以在代码级别违反单一职责原则;只有类中的方法足够少,才可以在方法级别保持单一原则
...
4. 里氏代换原则（Liskov Substitution Principle）
   ...
   > 将父类和子类都继承一个更通俗的类,原有的继承关系去掉,代用依赖,聚合,组合,等关系代替
